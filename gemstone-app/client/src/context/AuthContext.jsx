...
//     const { data } = await api.post('https://gemstone-recommendation-app.onrender.com/api/auth/login', { email, password });
...
//     const { data } = await api.post('https://gemstone-recommendation-app.onrender.com/api/auth/register', payload);
//     localStorage.setItem('gemstone_token', data.token);
//     localStorage.setItem('gemstone_user', JSON.stringify(data.user));
//     setUser(data.user);
//     return data;
...
export const AuthProvider = ({ children }) => {
  const [user, setUser] = useState(() => {
    try {
      return JSON.parse(localStorage.getItem('gemstone_user'));
    } catch {
      return null;
    }
  });
